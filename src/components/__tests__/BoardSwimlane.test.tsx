...
import handleCardMove from '../BoardSwimlane';
...
describe('BoardSwimlane', () => {
  let updateWorkItemSpy: jest.SpyInstance<Promise<WorkItem>, [orgId: number, workItemId: number, workItem: Partial<Pick<WorkItem, "name" | "status">>]>;
  let getWorkItemsSpy: jest.SpyInstance<Promise<WorkItem[]>, [orgId: number, filter?: Partial<Pick<WorkItem, "type" | "parentId">>]>;
  ...
  beforeEach(() => {
    ...
    getWorkItemsSpy = jest.spyOn(
      WorkitemService,
      'getWorkItems'
    );
    
    getWorkItemsSpy.mockResolvedValue([
      mockWorkItem
    ]);
  });
  ...
  test('notify when work item patch request success', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(`Feature name changed to: "new feature name".`)
      ).toBeInTheDocument();
    });
  });
  ...
  test('notify when work item patch request fails', async () => {
    ...
    updateWorkItemSpy.mockRejectedValue({ errors: ['ErrorMessage'] });
  ...
  /*
    ...
    *** I have no idea how to simulate drag and drop lol ***
    ...
    test('moving card to same lane does nothing', async () => {
      ...
      expect(updateWorkItemSpy).toBeCalledTimes(0);
    });
  */
});